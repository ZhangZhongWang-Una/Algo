...
def Model(dnn_feature_columns, num_tasks, tasks, flags):
    ...
    mem_out = MemoryLayer(memory_size=flags.mem_size, dropout_rate=flags.dropout)(emb_out)
    ...
    # mmoe_outs = PLELayer(num_tasks, 2, flags.expert_num, flags.expert_dim)(dcn_out)
    dcn_out = DCNMLayer(low_rank=flags.dnn1, num_experts=flags.expert_num, dropout_rate=flags.dropout)(mem_out)
    mmoe_outs = CGCLayer(num_tasks, flags.expert_num, flags.expert_dim, flags.dropout)(dcn_out)
    task_outputs = []
